    ...
    <div class="container noPrint">
      <b-row>
        <b-col style="display: flex">
          <b style="margin: auto; padding-right: 10px">Clientes:</b>
          <b-input-group>
            <b-form-select
              v-model="selected"
              :options="customerx"
            ...
            <template #append>
              <b-btn variant="outline-secondary" v-b-modal.cusModal>
                <b-icon icon="plus"></b-icon> Add</b-btn
              >
            </template>
          </b-input-group>
      ...
      <b-row>
        <b-col cols="3">
          <div class="mt-3" style="display: flex">
            <b style="padding-right: 10px">Telefone: </b
            >{{ selected.contactNo }}
          </div>
        </b-col>
        <b-col>
          <div class="mt-3" style="display: flex">
            <b style="padding-right: 10px">Endereço: </b>{{ selected.address }}
          </div>
        </b-col>
        <b-col cols="2">
      ...
      <div>
        ...
        <br />
        <b-input
          placeholder="digite o numero de serie do producto"
          v-model="serial"
          @keyup.enter="onEnter"
        ></b-input>
        <b-alert
        ...
        <table class="table">
          ...
          <tbody>
            <tr v-for="(j, index) in cartx2" :key="index">
              ...
              <td>{{ j.quantity }}</td>
              <td>{{ j.quantity * j.price }}.00</td>
              <td>
                <b-btn @click="icreaseQuantity2(j, index)">+</b-btn>
              </td>
              <td>
        ...
        <div class="text-right pull-right">
          <b-btn @click="outx()" variant="outline-success">Confirmar</b-btn>
        </div>
      </div>
    </div>
  ...
  methods: {
    ...
    outx() {
      if (this.selected.id == null) {
        ...
        Swal.fire({
          title: "Tens a certesa  ?",
          text: "Não poderas reverter esta ação!",
          icon: "warning",
    ...
    icreaseQuantity2(j, index){
        if(j.quantity > j.stocks) {
            Toast.fire({
              icon: "error",
              title: "estoques restantes ",
              text: "A quantidade excede o estoque restante",
            });
      ...
      if (b.includes(this.returnvalue.serialnumber)) {
        if (Object.keys(this.returnvalue).length === 0) {
          ...
          Toast.fire({
            ...
            title: "Oops!",
            text:
              "O producto com número de série  " + this.serial + " já está no carrinho!",
          });
        } else {
          //add qty
        ...
        Toast.fire({
          icon: "error",
          title: "Número de série desconhecido",
        });
      }
    },
